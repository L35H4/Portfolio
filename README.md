...
`check` проверяет синтаксис JS, существование и регистр путей 28 локальных ресурсов, основные HTML-атрибуты, домен и отсутствие путей старой среды в коде сайта.
...
DOM-тесты jsdom не проверяют настоящую вёрстку. Перед обновлением проверяйте в браузере desktop/mobile, все три кейса, 16 слайдов, Enter/Tab/Shift+Tab/Esc, возврат прокрутки, резюме, контакты и 404. Не отправляйте реальные сообщения ради теста.
...
Альтернатива для уже согласованного локального коммита в `main`:
...
Первый запуск только проверяет. Второй запрашивает подтверждение конкретного SHA, требует чистую ветку `main`, проверяет origin, CNAME, ресурсы, тесты и отсутствие новых удалённых коммитов; затем отправляет именно проверенный SHA и сверяет его через `ls-remote`. Он не добавляет файлы и не создаёт коммиты автоматически. Для заранее согласованного SHA доступен параметр `-ExpectedCommit <полный-SHA>`.

Публикация всегда требует отдельного согласования владельца. Не используйте force push. Значение CNAME должно оставаться `www.korepanov.art`.
